...
const UserDrawer: React.FC<UserDrawerProps> = ({ 
  ...
  const getProductsByIds = (ids: string[]) => ids.map(id => products.find(p => p.id === id)).filter(Boolean) as Product[];
  ...
  // Handle Logout Logic injected via props or global (here simulated as direct reload or clear)
  const handleLogout = () => {
          ...
          <div className="grid grid-cols-2 gap-3">
             <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                ...
                <div>
                   ...
                   <div className="text-sm font-bold text-slate-900">{user.status}</div>
                </div>
             </div>
             <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
