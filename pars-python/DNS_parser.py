...
def parse_characteristics_html(html: str, url: str):
    """Parse product page from raw HTML (useful for non-Selenium fetchers)."""
    ...
    notebook["Категория"] = category.text.lstrip(': ') if category is not None else ''
    notebook["Наименование"] = name.text[15:] if name is not None else ''
    notebook["Цена"] = int(price.text.replace(' ', '')[:-1]) if price is not None else 0
    notebook["Доступность"] = avail.text if avail is not None else 'Товара нет в наличии'
    notebook["Ссылка на товар"] = url
    notebook["Описание"] = desc.text if desc is not None else ''
    notebook["Главное изображение"] = main_picture.get('src') if main_picture is not None else 'У товара нет картинок'
    notebook["Лист с картинками"] = pictures_list
    notebook["Характеристики"] = list(tech_spec.items())
